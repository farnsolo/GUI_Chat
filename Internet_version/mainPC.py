...
class GUI:
    def __init__(self):
        ...
        self.server_destroyBu = tk.Button(self.root, text="Destroy server", font=("Ariel", 16), command=lambda: self.stop_server(True))
    ...
    def create_client(self):
        ...
        try:
            serverListOption.Main(self.username)
                ...
                if e.errno == 10061:
                    messagebox.showerror("Error",f"Resolution server is offline, can not view servers {e}")
        except Exception as e:
            messagebox.showerror("Error", e)

# -----------------------------------------------------------------------------------------------------------------------------------------------------------

    def stop_server(self, window):
        if window:
            self.server_destroyBu["state"] = "disabled"
            self.server_createBu["state"] = "normal"
